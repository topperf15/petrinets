...
class PNet(object):
        ...
        if rule_type not in ['function']:
            for t in actions:
                ...
                if rule_type == 'delay': 
                    ...
                    self.edge_names.append((rule_name))
    ...
    def _step_rule(self, movement, value, interval):
        ...
        source_attrib = movement[0][1]
        destination_place = self.places[movement[1][0]]
        destination_attrib = movement[1][1]
        if source_place.attributes[source_attrib] < (value*interval) \
            and self.zerolowerbound == True:
            value = source_place.attributes[source_attrib]
        src_val = source_place.attributes[source_attrib]
        source_place.attributes[source_attrib]=src_val-(value*interval)
        dest_val = destination_place.attributes[destination_attrib]
        destination_place.attributes[destination_attrib]=dest_val+(value*interval)
        # src_val = source_place.attributes[source_attrib][1]
        # source_place.attributes[source_attrib][1]=src_val-(value*interval)
        # dest_val = destination_place.attributes[destination_attrib][1]
        # destination_place.attributes[destination_attrib][1]=dest_val+(value*interval)
        
        # source_place.attributes[source_value][1] = \
        #     source_place.attributes[source_value][1] - (value*interval)
        # destination_place.attributes[destination_value][1] = \
        #     destination_place.attributes[destination_value][1] + \
        #     (value*interval)
    ...
    def _incubate_rule(self, rule, interval):
        ...
        if len(['failed' for t in test if t == 'failed']) == 0:
            ...
            if token_to == 0:  #if none have timed out add interval to timer for all
    ...
    def _function_rule(self, movement, function, conditions):
        ...
        if len(['failed' for t in test if t == 'failed']) == 0:
            ...
            if source_place.attributes[source_value] < token_value \
                and self.zerolowerbound == True:
                token_value = source_place.attributes[source_value]
            source_place.attributes[source_value] = \
                source_place.attributes[source_value] - token_value
    ...
    def _get_marking(self):
        '''!
        Private method to generate current marking.
        '''
        rept = {}
        for pName in self.places.keys():
            for aName in self.places[pName].attributes.keys():
